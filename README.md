...
- the root `/global` menu in `General` now stacks `Bot Settings` / `Language` first, then `Guide` / `Help`, with `Wait` / `Suffix` and `Zoo` / `Clear` below; `Bot Settings` also owns the global model/reasoning controls for Spike, Omni, and `/compact`
