...
- `General` keeps one persistent global menu and can clean up its tracked clutter with `/clear`
- the optional `Zoo` topic is menu-only and reserved for experimental project tamagotchi cards
...
3. `src/telegram/command-router.js` authenticates the configured operator, handles topic commands, Zoo flows, General cleanup, and turns plain messages plus attachments into prompt input.
...
- file delivery is restricted to the current worktree, the session state directory, and the system temp dir
