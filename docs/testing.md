...
On native Windows, leave `CODEX_BIN_PATH` empty unless you need a custom shim path. The runtime now falls back to `codex.cmd` by default, the symlinked-worktree coverage uses directory junctions so Developer Mode is not required for that test slice, `scripts\windows\test-live.cmd` changes into the repo root before spawning the live suite, and guidebook/runbook PDF generation now prefers Unicode-capable system fonts from `%WINDIR%\Fonts` so Cyrillic does not degrade into mojibake.
...
- `node --test test/spike-update-dispatch.test.js test/telegram-prompt-attachments.test.js test/telegram-prompt-queue.test.js` — focused oversized-attachment poison-update recovery plus normal attachment and queue ingress behavior
...
- `node --test test/worker-pool.test.js test/worker-pool-startup.test.js test/worker-pool-file-delivery.test.js test/worker-pool-delivery.test.js test/worker-pool-live-steer.test.js test/worker-pool-shutdown.test.js` — focused worker-pool ownership slices for startup, delivery, live steer, shutdown, transient final-reply recovery, same-thread restarts after upstream abort, ordinary upstream-interrupt retries with a bounded two-retry cap, final-answer salvage after late upstream abort, per-attempt recovery telemetry, steer-image replay on recovery, partial-delivery metadata, and parked-topic final-send behavior
...
- keep oversized attachment poll-poison regression coverage in `test/spike-update-dispatch.test.js`
