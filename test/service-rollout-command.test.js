...
test("performServiceRollout refuses to chain a fresh rollout while one is still in progress", async () => {
  const calls = [];
...
test("performServiceRollout refuses to chain a fresh rollout while one is still requested", async () => {
  const calls = [];
  await assert.rejects(
    performServiceRollout({
      generationStore: {},
